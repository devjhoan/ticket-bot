...
client.on("interactionCreate", async (interaction) => {
	...
	if (interaction.isButton()) {
		if (interaction.customId == "Ticket-Transcript") {
			...
			if (!guildData.channelTranscript) return interaction.reply({
				content: `${mensajes['NO-TRANSCRIPT-CHANNEL']}`,
				ephemeral: true
			})
	...
	if (interaction.isButton()) {
		...
		if (interaction.customId == "TR-YES") {
			...
			const mensaje = new MessageEmbed()
				...
				.setColor("#2f3136")
			await client.channels.cache.get(transcriptcanal).send({
				embeds: [mensaje],
				files: [file]
			}).then((a) => {
			...
			try {
				...
				return interaction.message.channel.send({
					embeds: [new MessageEmbed().setDescription(`\n❌ Error: This user has closed MD`).setColor("RED")]
				})
			}
		...
		if (interaction.customId == "TR-NO") {
			...
			const mensaje = new MessageEmbed()
				.setAuthor(interaction.client.users.cache.get(idmiembro).tag, interaction.client.users.cache.get(idmiembro).avatarURL({
					dynamic: true
				}))
				.addField("Ticket Owner", `<@!${idmiembro}>`, true)
				.addField("Ticket Name", `${interaction.channel.name}`, true)
